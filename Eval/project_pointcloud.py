...
def transform_radar_to_camera(radar_points):
    """
    将雷达坐标系转换到相机坐标系
    雷达坐标系: X前, Y左, Z上
    相机坐标系: X右, Y下, Z前
...
def main():
    parser = argparse.ArgumentParser(description='将雷达点云投影到多视角图像')
    ...
    parser.add_argument('--output_dir_name', type=str, default='vis_pointcloud')
    ...
    for order_idx, frame_idx in enumerate(tqdm(frames, desc="处理进度")):
        ...
        for cam_name in args.cameras:
            ...
            cv2.putText(result, f'View: {cam_name} | Points: {len(points_2d_valid)}', (10, 70), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
